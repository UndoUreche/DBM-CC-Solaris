...
local warnBreath			= mod:NewSpellAnnounce(58956, 2, nil, "Tank|Healer")
...
local specWarnFireWall			= mod:NewSpecialWarning("WarningFireWall", nil, nil, nil, 2, 2, nil, nil, 43113)
...
local timerBreath			= mod:NewNextTimer(10, 58956, nil, "Tank|Healer", nil, 5)
local timerWall				= mod:NewNextTimer(25, 43113, nil, nil, nil, 2)
local timerTenebron			= mod:NewTimer(30, "TimerTenebron", 61248, nil, nil, 1)
local timerShadron			= mod:NewTimer(55, "TimerShadron", 58105, nil, nil, 1)
local timerVesperon			= mod:NewTimer(120, "TimerVesperon", 61251, nil, nil, 1)
local timerTenebronWhelps		= mod:NewTimer(27, "TimerTenebronWhelps", 1022)
local timerShadronPortal		= mod:NewTimer(15, "TimerShadronPortal", 11420)
local timerVesperonPortal		= mod:NewTimer(30, "TimerVesperonPortal", 57988)
local timerTenebronPortal		= mod:NewTimer(15.5, "TimerTenebronPortal", 57989)
...
local function CheckDrakes(self, delay)
	...
	if isunitdebuffed(DBM:GetSpellInfo(61248)) then	-- Power of Tenebron
		...
		timerTenebronPortal:Schedule(30-delay)
		if self.Options.HealthFrame then
			DBM.BossHealth:AddBoss(30452, "Tenebron")
		end
	end
	if isunitdebuffed(DBM:GetSpellInfo(58105)) then	-- Power of Shadron
		timerShadron:Schedule(30 - delay, 38.4)
		warnShadron:Schedule(66.4 - delay)
		timerShadronPortal:Schedule(68.4 - delay)
		warnShadronPortalSoon:Schedule(78.4 - delay)
		if self.Options.HealthFrame then
			DBM.BossHealth:AddBoss(30451, "Shadron")
		end
	end
	if isunitdebuffed(DBM:GetSpellInfo(61251)) then	-- Power of Vesperon
		timerVesperon:Schedule(68.4 - delay, 58)
		warnVesperon:Schedule(124.4, - delay)
		timerVesperonPortal:Schedule(126.5, - delay)
		warnVesperonPortalSoon:Schedule(151.5 - delay)
		if self.Options.HealthFrame then
...
function mod:SPELL_CAST_SUCCESS(args)
	if args:IsSpellID(59127) then --57579 is another id
...
function mod:CHAT_MSG_RAID_BOSS_EMOTE(msg, mob)
	...
	elseif (msg == L.Eggs or msg:find(L.Eggs)) and mob == L.NameTenebron then --This stays here until the CC devs fix the emotes
		self:SendSync("TenebronPortal")
	elseif (msg == L.Disciple or msg:find(L.Disciple) and mob == L.NameVesperon) then
...
function mod:UNIT_DIED(args)
	...
	if cid == 30452 then --tenebron
		timerTenebronPortal:Cancel()
		
		timerTenebronWhelps:Cancel() --killing tenebron stops the whelps from spawning
		warnTenebronWhelpsSoon:Cancel()

	elseif cid == 30451 then -- shadron
		timerShadronPortal:Cancel()
		warnShadronPortalSoon:Cancel()
...
function mod:OnSync(event)
	...
	elseif event == "TenebronPortal" then
		...
		timerTenebronWhelps:Start()
		warnTenebronWhelpsSoon:Schedule(22)

		timerTenebronPortal:Schedule(27, 33)
