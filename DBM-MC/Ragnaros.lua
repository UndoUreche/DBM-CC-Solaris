...
function mod:CHAT_MSG_MONSTER_YELL(msg)
	if msg == L.Submerge1 or msg == L.Submerge2 then
...
function mod:UNIT_DIED(args)
	if(args.sourceName == "Son of Flame") then
		sonCounter = sonCounter + 1
	end
	
	if sonCounter == 8 then
...
function mod:OnSync(msg, arg)
	...
	if msg == "Submerge" then
		self:ScheduleMethod(0, "submerge")
	elseif msg == "Emerge" then
		self:ScheduleMethod(0, "emerge")
	end
end